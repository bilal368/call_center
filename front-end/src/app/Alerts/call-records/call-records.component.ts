...
@Component({
  ...
  standalone: true, providers:[SearchPipe,{ provide: MAT_DIALOG_DEFAULT_OPTIONS, useValue: { autoFocus: false } },],
  imports: [MatCheckboxModule,FormsModule,MatIconModule,CommonModule,TranslateModule,
    MatButtonModule,MatMenuModule,SearchPipe,MatTooltipModule],
  
  templateUrl: './call-records.component.html',
  styleUrl: './call-records.component.css'
})
export class CallRecordsComponent {
  buildDiv: string = 'Record Deleted';
  userList:any=[
  ...
  requestBody: {
    ...
    alertItem: string;
    userId: string;
    userListwithAlerts: any[];}={ action: "",  alertItem: "",  userId: "", userListwithAlerts:[]}
...
removeUserFromList(alertUserId:any){
  ...
  usersForAdd() {
    this.dialog.open(AlertsManagementComponent,{data:{alertItem:this.buildDiv,type:'usersForAdd'},width:'40vw',height:'55vh'}).afterClosed().subscribe((result:any)=>{
      this.getUsersInAlerts( 'getUsersInAlerts', this.buildDiv)
    })
  }
